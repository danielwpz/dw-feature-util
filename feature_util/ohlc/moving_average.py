...
class SimpleMovingAverageFeatureProducer(FeatureProducerOHLC):
    ...
    def produce(self, df):
        ...
        result['%s_%s_%d' % (self.feature, self.feature_label, self.period)] = \
            result[self.feature].rolling(window=self.period, min_periods=self.min_period).mean()
        return result
...
class ExponentialMovingAverageFeatureProducer(FeatureProducerOHLC):
    ...
    def produce(self, df):
        result = df.copy()
        result['%s_%s_%d' % (self.feature, self.feature_label, self.period)] = \
            result[self.feature].ewm(span=self.period, min_periods=self.min_period, adjust=self.adjust).mean()
        return result
